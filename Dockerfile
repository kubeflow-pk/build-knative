...
COPY . .
RUN go mod vendor && \
    GO111MODULE=on go build -o /bin/activator cmd/activator/*.go && \
    GO111MODULE=on go build -o /bin/autoscaler-hpa cmd/autoscaler-hpa/*.go && \
    GO111MODULE=on go build -o /bin/autoscaler cmd/autoscaler/*.go && \
    GO111MODULE=on go build -o /bin/controller cmd/controller/*.go && \
    GO111MODULE=on go build -o /bin/networking-nscert cmd/networking/nscert/*.go && \
    GO111MODULE=on go build -o /bin/webhook cmd/webhook/*.go && \
    GO111MODULE=on go build -o /bin/queue cmd/queue/*.go && \
    GO111MODULE=on go build -o /bin/default-domain cmd/default-domain/*.go
